...
@Repository
public interface IdTypeRepository extends JpaRepository<IdType, Long>, JpaSpecificationExecutor<IdType> {
    ...
    @Query("SELECT i FROM IdType i WHERE ((:name IS NULL) OR (:name IS NOT NULL AND i.name LIKE %:name%))")
